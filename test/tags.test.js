...
describe('Noteful API - Tags', function () {
  ...
  describe('GET /api/tags/:id', function () {
    ...
    it('should throw 404 with message Not Found if tag is not owned by user', function () {
      let data;
      return Tag.findOne({userId: user2.id})
        .then(_data => {
          data = _data;
          return chai.request(app).get(`/api/tags/${data.id}`).set('Authorization', `Bearer ${token}`);
        })
  ...
  describe('PUT /api/tags/:id', function () {
    ...
    it('should allow user to update name of own tag with name owned by another user', function() {
      const updateItem = {};
      const id = '222222222222222222222225';
      const newItem = {'name' : 'new tag!!', userId: user2.id, _id: id};
      return Promise.all([
        Tag.create(newItem),
