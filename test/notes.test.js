...
describe('Noteful API - Notes', function () {
  ...
  describe('GET /api/notes/:id', function () {
    ...
    it('should get 404 Not Found when trying to access notes not owned by user', function() {
      let data;
  ...
  describe('PUT /api/notes/:id', function () {
    ...
    it('should update the note when provided a valid title', function () {
      ...
      return Note.findOne({userId: user.id})
        ...
        .then(function (res) {
          ...
          expect(res.body.tags[0].id).to.deep.equal(data.tags[0].toString());
          expect(new Date(res.body.createdAt)).to.eql(data.createdAt);
    ...
    it('should update the note when provided valid content', function () {
      ...
      let data;
      return Note.findOne({userId: user.id, tags : {$exists: true}})
        .then(_data => {
          data = _data;
          return chai.request(app)
            .put(`/api/notes/${data.id}`)
        ...
        .then(function (res) {
          ...
          expect(res.body.folderId).to.equal(data.folderId.toString());
          res.body.tags.forEach((tag, idx) => {
            expect(tag.id).to.eql(data.tags[idx].toString());
          });
          expect(new Date(res.body.createdAt)).to.eql(data.createdAt);
          // expect note to have been updated
          expect(new Date(res.body.updatedAt)).to.greaterThan(data.updatedAt);
    ...
    it('should return 404 Not Found if user updates note owned by another user', () => {
      const updateItem = {
        title: 'What about dogs?!',
        content: 'Lorem ipsum dolor sit amet, sed do eiusmod tempor...'
      };
      return Note.findOne({ folderId: { $exists: true } , userId: user2.id})
        .then((note) => {
          return chai.request(app)
            .put(`/api/notes/${note.id}`)
            .set('Authorization', `Bearer ${token}`)
